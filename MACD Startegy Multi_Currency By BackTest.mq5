...
//+------------------------------------------------------------------+
//| Download all historical data for all symbols in Symbols[]        |
//+------------------------------------------------------------------+
void DownloadAllHistory()
{
   for(int i=0; i<SymbolCount; i++)
   {
      ...
      {
         LogError("Failed to select symbol: " + sym);
         continue;
      }
      // Download all available bars for the timeframe
      MqlRates rates[];
      int bars = CopyRates(sym, TF, 0, 1000, rates); // Try to trigger download
      int total = Bars(sym, TF);
      Print("Downloaded ", total, " bars for ", sym);
   }
}
...
int OnInit()
{
   ...
   DownloadAllHistory(); // <-- Ensure history is loaded for all symbols
