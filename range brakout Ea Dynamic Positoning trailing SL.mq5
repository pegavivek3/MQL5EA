...
void UpdateStopLoss(){
   ...
   //return if no stoploss or fixed stop loss
   ...
   for(int i=total-1;i>=0;i--){
      ...
      if(ticket <= 0){Print("Failed get position ticket"); return;}
      if(!PositionSelectByTicket(ticket)){Print("Failed to get positon by ticket"); return;}
      long magicnumber;
      ...
      if(InpMagicNumber == magicnumber){
         ...
         if(!PositionGetDouble(POSITION_SL,currSL)){Print("Failed to get current stoploss"); return;}
         if(!PositionGetDouble(POSITION_TP,currTP)){Print("Failed to get current TakeProfit"); return;}
         ...
         //check if new SL is closer to existing cuurent Price than existing SL
         ...
         if(!trade.PositionModify(ticket,newSL,currTP)){
            Print("Failed to modify position, ticket: ",(string)ticket,
                  "current SL: ",string(currSL),"new SL: ",string(newSL),"Take Profit: ",string(currTP));
            return;
         }
      }
   }
...
int CountOpenPositions(){
   ...
   for(int i=total-1;i>=0;i--){
      ...
      long magicnumber;
      if(PositionGetInteger(POSITION_MAGIC,magicnumber)==false){Print("Failed to get position MagicNumber"); return -1;}
